...
| 参数                            | 含义 | 使用时或说明                       |
| ------------------------------- | ---- | ---------------------------------- |
| project(name **VERSION** 2.5)   | 版本 | message(STATUS ${PROJECT_VERSION}) |
| project(name **LANGUAGES** CXX) | 语言 | #CXX代表C++                        |
...
用于设置选项值，可以让用户在执行构建项目时控制宏的开启和关闭。
...
option(PRINT_VERSION "Whether print version or not" ON)
option(THIRD_TEST "Whether use third_party function or not" OFF)
...
用于在工程 `CMakeLists.txt` 中控制代码的开启和关闭.

- CMakeLists.txt

```cmake
add_definitions( -DDESKTOP -DLINK )
```
- main.cpp

```c++
#ifdef DESKTOP
cout << "this is desktop version" << endl;
#endif
```
